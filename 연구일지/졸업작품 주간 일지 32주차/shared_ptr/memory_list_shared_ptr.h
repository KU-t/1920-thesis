...
namespace LFSP {
	...
	public:
		cb_memory_helper()
		{
			...
			cb_list_head = new Node();
			cb_list_head->next = new Node();
			cb_list_tail = cb_list_head->next;

			n_list_head = new Node();
			n_list_head->next = new Node();
			n_list_tail = n_list_head->next;
		}
		...
		Node* Get_Recycle_node()
		{
			...
			while (true)
			{
				...
				if (true == CAS(&n_list_head, curr_h, curr_h->next))
				{
					return curr_h;
				}
			}
		...
		void Set_Recycle_node(Node* p)
		{
			...
			while (true)
			{
				...
				if (true == CAS(&n_list_tail, curr_t, p))
				{
					curr_t->next = p;
					return;
				}
		...
		void* Get_Recycle_control_block()
		{
			int _size = size;
			...
			while (true)
			{
				...
				if (true == CAS(&cb_list_head, curr_h, curr_h->next))
				{
					Set_Recycle_node(curr_h);
					size--;
					return curr_h->node;
				}
			}
		...
		void Set_Recycle_control_block(void* p)
		{
			Node* new_node;
			Node* curr;

			new_node = Get_Recycle_node();
			...
			{
				curr = n_list_tail;
				if (true == CAS(&n_list_tail, curr, new_node))
				{
					curr->next = new_node;
					size++;
					return;
		...
		void destroy(std::false_type)
		{
			...
			if (curr_weak_count == 1)
			{
				if (true == CAS(&weak_count, curr_weak_count, curr_weak_count - 1))
					hp->Set_Recycle_control_block(this);
			}
		}
	...
	shared_ptr<_Tp> make_shared(cb_memory_helper* hp, Args&&... _Args)
	{
		...
		void* ret = hp->Get_Recycle_control_block();
