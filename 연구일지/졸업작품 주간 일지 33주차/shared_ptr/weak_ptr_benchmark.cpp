...
LFSP::shared_ptr<Node>	shared[Num_shared_ptr];
LFSP::weak_ptr<Node>	weak[Num_shared_ptr * Num_weak_per_shared];
int Num_of_weak_of_shared[Num_shared_ptr];

void sp_thread_func(int num_thread) {
	int index1, index2;
	LFSP::weak_ptr<Node> empty;
	...
	LFSP::shared_ptr<Node> s = weak[index1].lock();
}

void main() {

	// Init
	LFSP::FreeList* fl = new LFSP::FreeList();
	for (int i = 0; i < Num_shared_ptr; ++i)
		shared[i] = LFSP::make_shared<Node>(fl, Node(i));
