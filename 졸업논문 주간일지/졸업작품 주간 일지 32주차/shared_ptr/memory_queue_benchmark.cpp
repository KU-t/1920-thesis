...
#include "memory_queue_shared_ptr.h"
...
public:
	SPCLIST() {
		hp = new LFSP::cb_memory_helper();
		head = LFSP::make_shared<SPNODE>(hp, 0x80000000);
		tail = LFSP::make_shared<SPNODE>(hp, 0x7fffffff);
		head->next = tail;
		...
		while (true) {
			...
			if (validate(pred, curr)) {
				if (key == curr->key) {
					...
					LFSP::shared_ptr<SPNODE> node = LFSP::make_shared<SPNODE>(hp, key);
