...
public:
	SPCLIST() {
		fl = new LFSP::FreeList();
		head = LFSP::make_shared<SPNODE>(fl, 0x80000000);
		tail = LFSP::make_shared<SPNODE>(fl, 0x7fffffff);
		head->next = tail;
		...
		while (true) {
			...
			if (validate(pred, curr)) {
				if (key == curr->key) {
					...
					LFSP::shared_ptr<SPNODE> node = LFSP::make_shared<SPNODE>(fl, key);
